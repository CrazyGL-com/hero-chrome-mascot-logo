...
function ChromeMascotLogoHero(props) {
    const { size, input, reducedMotion, rootRef, layout = 'content-left', logo = 'https://crazygl.com/samples/logo-shape1.png', useGlbMaterials = false, bevelDepth = 0.25, logoColor = '#f6f8fc', logoMetalness = 0.92, logoRoughness = 0.125, clearcoatStrength = 1.0, envMapIntensity = 1.4, logoScale = 0.87, pointerStrength = 1.0, autoRotateSpeed = 0.045, envPreset = 'Sunset', bgStyle = 'sunset-chrome', bgMotion = 1.0, gradientTop = '#5beaff', gradientMiddle = '#ff5fb1', gradientBottom = '#ffc56b', starCount = 40, starColor = '#fff7e6', starDriftSpeed = 0.4, flareStrength = 0.6, flareColor = '#fff2c8', } = props;
    const content = useContent(props);
    useHeroReady(props);
    const [mounted, setMounted] = React.useState(false);
    React.useEffect(() => setMounted(true), []);
    const contentAlign = layout === 'content-left'
    ...
    return (_jsxs(_Fragment, { children: [_jsx("crazygl-stage", { style: {
                    ...
                    overflow: 'hidden',
                    background: '#0a0612',
                }, children: mounted ? (_jsx(React.Suspense, { fallback: null, children: _jsx(ThreeStage, { rootRef: rootRef, size: size, input: input, reducedMotion: reducedMotion, layout: layout, logo: logo, useGlbMaterials: useGlbMaterials, bevelDepth: bevelDepth, logoColor: logoColor, logoMetalness: logoMetalness, logoRoughness: logoRoughness, clearcoatStrength: clearcoatStrength, envMapIntensity: envMapIntensity, logoScale: logoScale, pointerStrength: pointerStrength, autoRotateSpeed: autoRotateSpeed, envPreset: envPreset, bgStyle: bgStyle, bgMotion: bgMotion, gradientTop: gradientTop, gradientMiddle: gradientMiddle, gradientBottom: gradientBottom, starCount: starCount, starColor: starColor, starDriftSpeed: starDriftSpeed, flareStrength: flareStrength, flareColor: flareColor }) })) : null }), _jsx("crazygl-content", { style: {
                    position: 'absolute',
                    inset: 0,
                    display: 'flex',
                    alignItems: 'center',
                    ...
                    pointerEvents: 'none',
                    ...contentAlign,
                }, children: _jsx("div", { className: "crazygl-cml-content", children: content.node }) })] }));
}
export { metadata };
export default function ChromeMascotLogo(props) {
    return _jsx(CrazyGLWrapper, { hero: ChromeMascotLogoHero, metadata: metadata, ...props });
}
