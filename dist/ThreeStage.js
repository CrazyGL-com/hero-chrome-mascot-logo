...
function buildHDRTexture(preset) {
...
/* ──────────────────────────────────────────────────────────────────────
   ...
   ────────────────────────────────────────────────────────────────────── */
function makeFlareTexture(color) {
    const W = 1024;
    const H = 128;
    const cv = document.createElement('canvas');
    cv.width = W;
...
export default function ThreeStage(props) {
    const { rootRef, size, input, reducedMotion, layout, logo, useGlbMaterials, bevelDepth, logoColor, logoMetalness, logoRoughness, clearcoatStrength, envMapIntensity, logoScale, pointerStrength, autoRotateSpeed, envPreset, bgStyle, bgMotion, gradientTop, gradientMiddle, gradientBottom, starCount, starColor, starDriftSpeed, flareStrength, flareColor, } = props;
    ...
    const glbMatTrackedRef = React.useRef(new Set());
    const bgMeshRef = React.useRef(null);
    const bgMatRef = React.useRef(null);
    const starsRef = React.useRef(null);
    const starMatRef = React.useRef(null);
    const starDataRef = React.useRef(null);
    const starTexRef = React.useRef(null);
    const flareGroupRef = React.useRef(null);
    const flareSpritesRef = React.useRef([]);
    const flareTexRef = React.useRef(null);
    ...
    React.useEffect(() => {
        ...
        if (!mat)
            return;
        mat.uniforms.u_top.value.set(gradientTop);
        mat.uniforms.u_mid.value.set(gradientMiddle);
        mat.uniforms.u_bot.value.set(gradientBottom);
    ...
    React.useEffect(() => {
        ...
        mat.uniforms.u_accentA.value.setRGB(acc.a[0], acc.a[1], acc.a[2]);
        mat.uniforms.u_accentB.value.setRGB(acc.b[0], acc.b[1], acc.b[2]);
        mat.uniforms.u_accentC.value.setRGB(acc.c[0], acc.c[1], acc.c[2]);
    }, [bgStyle]);
    ...
    const ensureChromeMaterial = React.useCallback(() => {
    ...
    React.useEffect(() => {
        ...
        const handle = setTimeout(() => {
            ...
            const curveSegments = Math.max(14, Math.round(30 / Math.sqrt(bevelSegments + 1)));
    ...
    React.useEffect(() => {
        ...
        if (!mat)
            return;
        mat.uniforms.u_color.value.set(starColor);
    }, [starColor]);
    React.useEffect(() => {
        const mat = starMatRef.current;
        if (!mat)
    ...
    useHeroAnimationFrame(rootRef, ({ delta, elapsed }) => {
        ...
        const layoutX = layout === 'content-left' ? 0.9 : layout === 'content-right' ? -0.9 : 0.0;
        ...
        group.rotation.set(pitchRef.current, autoYawRef.current + yawRef.current, 0);
        // ─── Stars — twinkle + drift ──────────────────────────────────
        const stars = starsRef.current;
        const data = starDataRef.current;
        if (stars && data) {
            const geom = stars.geometry;
            const posAttr = geom.attributes.position;
            const sizeAttr = geom.attributes.a_size;
            const opAttr = geom.attributes.a_opacity;
            const t = elapsed;
        ...
        if (flares.length && states.length) {
            for (let i = 0; i < flares.length; i++) {
                ...
                const env = Math.max(0, 0.5 + 0.5 * Math.sin(elapsed * 0.18 + st.phase * 1.7));
    ...
    return (_jsx("canvas", { ref: canvasRef, className: "crazygl-cml-canvas", "aria-hidden": "true" }));
}
